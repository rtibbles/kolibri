/**
 * Kolibri data API shim for HTML5 zip content.
 *
...
import { events, nameSpace, DataTypes } from 'kolibri-sandbox/base';
 ...
 * custom apps should handle the as it may be used to
 * generate links externally to jump to this state
 */

export default class KolibriShim extends SandboxShim {
  static shimName = 'kolibri';

  constructor(mediator) {
    super(mediator);
    this.data = {};
    // Create a new mediator for the parent window for Kolibri API calls
    this.parentMediator = new Mediator(window.parent);
  }
  ...
  __setShimInterface() {
    ...
    class Shim {
      ...
      getContentByFilter(options) {
        return self.parentMediator.sendMessageAwaitReply({
