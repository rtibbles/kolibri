...
describe('MoreToExploreSection', () => {
  it('renders nothing when groups is empty', () => {
    ...
    expect(wrapper.find('[data-testid="more-to-explore"]').exists()).toBe(false);
  });

  it('renders section when groups are provided', () => {
    const wrapper = makeWrapper({ groups: mockGroups });
    expect(wrapper.find('[data-testid="more-to-explore"]').exists()).toBe(true);
  });
  ...
  it('renders a group for each category', () => {
    const wrapper = makeWrapper({ groups: mockGroups });
    const groups = wrapper.findAll('[data-testid="explore-group"]');
    expect(groups.length).toBe(2);
  });

  it('renders group label and count', () => {
    const wrapper = makeWrapper({ groups: mockGroups });
    const firstGroup = wrapper.findAll('[data-testid="explore-group"]').at(0);
