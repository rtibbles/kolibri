...
import { isNumber, prefixed, borders, defaults } from 'epubjs/src/utils/core';
import EpubCFI from 'epubjs/src/epubcfi';
import Mapping from 'epubjs/src/mapping';
import { replaceLinks } from 'epubjs/src/utils/replacements';
import { EPUBJS_VERSION, EVENTS, DOM_EVENTS } from 'epubjs/src/utils/constants';
import logger from 'kolibri.lib.logging';

const logging = logger.getLogger(__filename);

function imageLoadListeners() {
  ...
  for (var i = 0; i < images.length; i++) {
    ...
    if (typeof img.naturalWidth !== 'undefined' && img.naturalWidth === 0) {
      img.onload = window.postMessage('IMAGE_RESIZE');
    }
  }
}
...
class SandboxContents {
  constructor(doc, content, cfiBase, sectionIndex, view) {
    this.view = view;
   ...
   */
  imageLoadListeners() {
    const fn = imageLoadListeners.toString();
    const handler = data => {
      if (data === 'IMAGE_RESIZE') {
        this.expand();
      }
    };
    this.view.registerHandler(handler);
    this.view.sendCommand(fn);
  }
...
EventEmitter(SandboxContents.prototype);

export default SandboxContents;
